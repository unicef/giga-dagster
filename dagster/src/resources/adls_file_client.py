...
class ADLSFileClient:
    ...
    def move_file_in_adls(self, origin_filepath: str, destination_filepath: str):
        file_client = self.adls.get_file_client(origin_filepath)
        destination_url = f"https://{AZURE_BLOB_SAS_HOST}/{AZURE_BLOB_CONTAINER_NAME}/{destination_filepath}{AZURE_SAS_TOKEN}"
        print(f"DEST: {destination_url}")
        file_client.rename_file(destination_url)
