...
def list_bronze(context: OpExecutionContext, list_raw: sql.DataFrame) -> pd.DataFrame:
    ...
    yield Output(
        list_raw.toPandas(), metadata={"filepath": get_output_filepath(context)}
    )
...
def list_dq_passed_rows(
    context: OpExecutionContext,
    list_data_quality_results: sql.DataFrame,
) -> sql.DataFrame:
    df_passed = list_data_quality_results
...
def list_dq_failed_rows(
    ...
    df_failed = list_data_quality_results
