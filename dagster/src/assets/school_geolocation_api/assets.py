...
def mng_school_geolocation_api_raw(context: OpExecutionContext,
    ...
    def pull_data_from_api(last_update_date="2019-10-26T00:00:00Z"):
        ...
        if last_update_date:
            base_url = f"{settings.MONGOLIA_API_URL}?size=100&UPDATED_AFTER={last_update_date}"
        else:
    ...
    adls_file_client.upload_raw(context, data=schools_df.to_csv(index=False).encode(), filepath=adls_folder_path, metadata=metadata)
