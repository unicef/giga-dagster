...
import requests
from schemas.qos import SchoolListConfig
...
def query_school_list_API_data(
    context: OpExecutionContext, database_session: Session, row_data: SchoolListConfig
) -> list:
    ...
    if row_data["pagination_type"] is None:
        try:
            ...
            update_statement = (
                update(SchoolListConfig)
                .where(SchoolListConfig.id == row_data["id"])
