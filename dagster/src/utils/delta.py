...
def check_table_exists(spark: SparkSession, schema_name: str, table_name: str, data_tier: DataTier = None) -> bool:
    tiered_schema_name = construct_schema_name_for_tier(
        schema_name,
        data_tier,
    )
    table_path = f"{settings.SPARK_WAREHOUSE_DIR}/{tiered_schema_name}.db/{table_name.lower()}"
