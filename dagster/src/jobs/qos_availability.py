...
@op
def process_availability(context: OpExecutionContext, spark: PySparkResource):
    ...
    events_df = s.read.table(QOS_AVAILABILITY_VCT).select(
    ...
    if last_processed_ts is not None:
        ...
        events_df = events_df.filter(
            F.col("timestamp") > (F.lit(last_processed_ts) - F.expr("INTERVAL 2 HOURS"))
        )
    ...
    if last_processed_ts is not None:
        previous_df = (
            ...
            .filter(
                F.col("timestamp")
                <= (F.lit(last_processed_ts) - F.expr("INTERVAL 2 HOURS"))
            )
            .withColumn(
                "rn",
                F.row_number().over(
    ...
    last_complete_window = F.date_trunc(
        "hour", F.current_timestamp() - F.expr("INTERVAL 1 HOUR")
    )
    ...
    combined_df = combined_df.withColumn(
        "next_ts", F.lead("timestamp").over(window)
    ...
    combined_df = combined_df.withColumn(
        ...
        F.when(
            F.col("status") == "online",
            F.when(
                F.col("next_status") != "offline",
                F.coalesce(F.col("next_ts"), last_complete_window),
    ...
    online_df = online_df.withColumn(
        "start_hour", F.date_trunc("hour", "timestamp")
    ...
    hourly_df = (
        online_df.withColumn("hour_start", F.explode("hour_window"))
        .withColumn("hour_end", F.expr("hour_start + INTERVAL 1 HOUR"))
        .withColumn(
            "interval_start", F.greatest(F.col("timestamp"), F.col("hour_start"))
        )
        .withColumn("interval_end", F.least(F.col("end_ts"), F.col("hour_end")))
        .withColumn(
            "seconds_online",
            F.unix_timestamp("interval_end") - F.unix_timestamp("interval_start"),
        )
    )
    ...
    events_hourly = combined_df.withColumn(
        "hour_start", F.date_trunc("hour", "timestamp")
    )
    events_count = events_hourly.groupBy("device_id", "hour_start").agg(
        F.count("*").alias("count_state")
    )
    ...
    device_hours_df = (
        devices.withColumn("start_ts", F.lit(start_ts))
    ...
    hourly_full = (
        device_hours_df.join(hourly_agg, ["device_id", "timestamp"], "left")
        .join(
            events_count.select(
                "device_id", F.col("hour_start").alias("timestamp"), "count_state"
            ),
            ["device_id", "timestamp"],
            "left",
        )
    ...
    hourly_full = hourly_full.withColumn(
        "uptime_percentage",
        F.when(
            (F.col("count_state") == 5) & (F.col("uptime_percentage") == 0), 0
    ...
    if not DeltaTable.isDeltaTable(s, QOS_VCT):
        ...
        enriched.write.format("delta").mode("overwrite").partitionBy("date").option(
