...
#         for key, value in api.__dict__.items():
#             if isinstance(value, Enum):
#                 school_connectivity_config[key] = value.value
#             elif isinstance(value, datetime):
#                 school_connectivity_config[key] = value.strftime("%Y-%m-%d %H:%M:%S")
#             elif key == "school_list":
#                 for key2, value2 in api.school_list.__dict__.items():
#                     if isinstance(value2, Enum):
#                         school_list_config[key2] = value2.value
#                     elif isinstance(value2, datetime):
#                         school_list_config[key2] = value2.strftime("%Y-%m-%d %H:%M:%S")
#                     else:
#                         school_list_config[key2] = value2
#             else:
#                 school_connectivity_config[key] = value

#         school_connectivity_config["school_list"] = school_list_config

#         row_data = SchoolConnectivityConfig(**school_connectivity_config)
#         context.log.info(f">> ROW: {row_data}")

#         country_code = "RWA"
#         metastore_schema = "qos"
#         stem = f"{row_data.school_list.name}_{country_code}_{DOMAIN_DATASET_TYPE}_{scheduled_date.replace(' ', '_').replace(':', '').replace('-', '')}"
...
#                 tier=DataTier.DATA_QUALITY_CHECKS,
#             ),
#             "qos_school_connectivity_silver": OpDestinationMapping(
#                 source_filepath=f"{constants.dq_results_folder}/{DOMAIN}/{DATASET_TYPE}/dq-passed-rows/{country_code}/{stem}.csv",
#                 destination_filepath=f"{settings.SPARK_WAREHOUSE_PATH}/{metastore_schema}_silver.db/{country_code.lower()}",
#                 metastore_schema=metastore_schema,
#                 tier=DataTier.DATA_QUALITY_CHECKS,
#             ),
#             "qos_school_connectivity_gold": OpDestinationMapping(
#                 source_filepath=f"{settings.SPARK_WAREHOUSE_PATH}/{metastore_schema}_silver.db/{country_code.lower()}",
