...
@sensor(
    job=school_master_coverage__post_manual_checks_job,
    minimum_interval_seconds=settings.DEFAULT_SENSOR_INTERVAL_SECONDS,
)
def school_master_coverage__post_manual_checks_sensor(
    context: SensorEvaluationContext,
    adls_file_client: ADLSFileClient,
):
    count = 0
        ...
        try:
            ...
            ops_destination_mapping = {
                ...
                "manual_review_failed_rows": OpDestinationMapping(
                    source_filepath=str(path),
                    destination_filepath=f"{settings.SPARK_WAREHOUSE_PATH}/school_coverage_rejected.db/{country_code.lower()}",
