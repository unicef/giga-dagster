-- Appends new rows into default.superset_activity_logs from Superset logs.
-- Inserts every source id that is not already stored, with user, dashboard,
-- and chart context.


-----superset activity logs------------

INSERT INTO default.superset_activity_logs 
...
SELECT
    ...
    CAST(l.dttm AS DATE) AS log_date
...
-- Insert any source id that is not already in the destination table.
WHERE NOT EXISTS (
      SELECT 1
      FROM default.superset_activity_logs existing
