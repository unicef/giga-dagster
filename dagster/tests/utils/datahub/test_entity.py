...
@patch("src.utils.datahub.entity.get_datahub_graph_client")
def test_delete_entity_with_references_soft(mock_graph, mock_context):
    mock_graph.return_value.delete_references_to_urn.return_value = (5, [])
    urn = "urn:li:dataset:test"

    count = delete_entity_with_references(mock_context, urn, hard_delete=False)

    assert count == 5
    mock_graph.return_value.delete_references_to_urn.assert_called_with(
        urn=urn, dry_run=False
    )
    mock_graph.return_value.soft_delete_entity.assert_called_with(urn=urn)
    mock_graph.return_value.hard_delete_entity.assert_not_called()
...
def test_delete_entity_with_references_hard(mock_graph, mock_context):
    mock_graph.return_value.delete_references_to_urn.return_value = (0, [])
    urn = "urn:li:dataset:test"
    ...
    mock_graph.return_value.hard_delete_entity.assert_called_with(urn=urn)
    mock_graph.return_value.soft_delete_entity.assert_not_called()
...
def test_get_entity_count_safe_pagination(mock_graph):
    batch_size = 100
    mock_graph.return_value.list_all_entity_urns.side_effect = [
        ["urn"] * batch_size,
        ["urn"] * batch_size,
        ["urn"] * 50,
    ]
    ...
    assert total == 250
    assert mock_graph.return_value.list_all_entity_urns.call_count == 3
    # Check calls
    mock_graph.return_value.list_all_entity_urns.assert_any_call(
...
def test_get_entity_count_safe_retry_failure(mock_graph, capsys):
    mock_graph.return_value.list_all_entity_urns.side_effect = Exception(
        "Persistent Fail"
    )
...
def test_get_entity_count_safe_safety_limit(mock_graph, capsys):
    ...
    mock_graph.return_value.list_all_entity_urns.return_value = ["urn"] * large_batch
